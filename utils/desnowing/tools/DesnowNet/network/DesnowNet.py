...
class TR_new(nn.Module):
    ...
    def __init__(self, input_channel=3, beta=4, gamma=4):
        ...
        self.SE = Pyramid_maxout(960, 1, beta)
        self.AE = Pyramid_maxout(960, 3, beta)
...
class TR_za(nn.Module):
    ...
    def __init__(self, input_channel=3, beta=4, gamma=4):
        ...
        self.D_t = Descriptor(input_channel, gamma)
        self.SE = Pyramid_maxout(960, 1, beta)
        self.SAE = Pyramid_maxout(960, 3, beta)
...
class RG(nn.Module):
    ...
    def __init__(self, input_channel=7, beta=4, gamma=4):
        ...
        block = []
        for i in range(beta):
            block.append(nn.Conv2d(960, 3, 2 * i + 1, 1, padding=i))
