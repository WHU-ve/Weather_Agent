...
def colorize_mask(mask):
    ...
    new_mask = []
    for i in range(c):
        new_mask.append(Image.fromarray(mask[i].astype(np.uint8)).convert('P'))
        new_mask[i].putpalette(palette)
        # new_mask[i].save(os.path.join('pic', 'color_mask{}.png'.format(i)))
    # print(mask.shape)
    # new_mask = Image.fromarray(mask.astype(np.uint8)).convert('P')
    # new_mask.putpalette(palette)
    # print(len(new_mask.split()))
    return new_mask
