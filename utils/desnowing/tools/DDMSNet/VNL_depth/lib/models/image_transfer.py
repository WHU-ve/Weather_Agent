import sys, os
_VNL_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if _VNL_ROOT not in sys.path:
    sys.path.insert(0, _VNL_ROOT)
import torch
import cv2
...
def bins_to_depth(depth_bin):
    ...
    if type(depth_bin).__module__ != torch.__name__:
        depth_bin = torch.tensor(depth_bin, dtype=torch.float32)
    depth_bin = depth_bin.permute(0, 2, 3, 1) #[b, h, w, c]
    if type(cfg.DATASET.DEPTH_BIN_BORDER).__module__ != torch.__name__:
        cfg.DATASET.DEPTH_BIN_BORDER = torch.tensor(cfg.DATASET.DEPTH_BIN_BORDER, dtype=torch.float32)
        #print(cfg.DATASET.DEPTH_BIN_BORDER.device)
    # depth_bin = depth_bin.cuda(device='cuda:0')
    # print("depth_bin device:{}".format(depth_bin.device))
    # print("depth_bin_border device:{}".format(cfg.DATASET.DEPTH_BIN_BORDER.device))
    bin_border = cfg.DATASET.DEPTH_BIN_BORDER.to(depth_bin.device)
    depth = depth_bin * bin_border
