...
def generator(input):
    ...
    with tf.variable_scope("g_conv0"):
        convolved = tf.layers.conv2d(input, filters[0], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        layers.append(convolved)
        
    # layer 1
    with tf.variable_scope("g_conv1"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[1], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
    ...
    with tf.variable_scope("g_conv2"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[2], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_conv3"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[3], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_conv4"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[4], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_conv5"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[5], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_conv6"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[6], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_conv7"):
        rectified = tf.nn.leaky_relu(layers[-1], 0.2)
        convolved = tf.layers.conv2d(rectified, filters[7], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv0"):
        ...
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[8], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
    ...
    with tf.variable_scope("g_deconv1"):
        ...
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[9], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv2"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[10], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv3"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[11], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv4"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[12], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv5"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[13], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv6"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, filters[14], kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(deconvolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        layers.append(normalized)
    ...
    with tf.variable_scope("g_deconv7"):
        ...
        rectified = tf.nn.relu(concatenated)
        deconvolved = tf.layers.conv2d_transpose(rectified, 3, kernel_size = 4, strides = (2, 2), padding = "same", kernel_initializer = tf.glorot_uniform_initializer())
...
def discriminator(inputs):
    ...
    with tf.variable_scope("d_conv0"):
        convolved = tf.layers.conv2d(inputs, filters[0], kernel_size = 3, strides = (1, 1), padding="same", kernel_initializer = tf.glorot_uniform_initializer())
    ...
    with tf.variable_scope("d_conv1"):
        convolved = tf.layers.conv2d(layers[-1], filters[1], kernel_size = 3, strides = (2, 2), padding="valid", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
    ...
    with tf.variable_scope("d_conv2"):
        convolved = tf.layers.conv2d(layers[-1], filters[2], kernel_size = 3, strides = (1, 1), padding="same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv3"):
        convolved = tf.layers.conv2d(layers[-1], filters[3], kernel_size = 3, strides = (2, 2), padding="valid", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv4"):
        convolved = tf.layers.conv2d(layers[-1], filters[4], kernel_size = 3, strides = (1, 1), padding="same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv5"):
        convolved = tf.layers.conv2d(layers[-1], filters[5], kernel_size = 3, strides = (2, 2), padding="valid", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv6"):
        convolved = tf.layers.conv2d(layers[-1], filters[6], kernel_size = 3, strides = (1, 1), padding="same", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv7"):
        convolved = tf.layers.conv2d(layers[-1], filters[7], kernel_size = 3, strides = (2, 2), padding="valid", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
    ...
    with tf.variable_scope("d_conv8"):
        convolved = tf.layers.conv2d(layers[-1], filters[8], kernel_size = 3, strides = (2, 2), padding="valid", kernel_initializer = tf.glorot_uniform_initializer())
        normalized = tf.layers.batch_normalization(convolved, axis = 3, epsilon = 1e-5, momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))
        rectified = tf.nn.leaky_relu(normalized, 0.2)
        #layers[8]
        layers.append(rectified)
