...
UTILS_DIR_NAME = os.environ.get('WEATHER_UTILS_DIR', 'utils_new')
...
class BasicSRModel(Tool):
    ...
    def _preprocess(self):
        ...
        cfg_path = Path().resolve() / 'utils' / self.subtask / 'configs' / f'{self.tool_name}.yml'
        ...
        # Optional runtime memory knobs for X-Restormer.
        if self.tool_name == 'xrestormer':
            tile_cfg = cfg.get('tile')
            if isinstance(tile_cfg, dict):
                tile_size = int(os.environ.get('WEATHER_XRESTORMER_TILE', str(tile_cfg.get('tile_size', 512))))
                tile_overlap = int(os.environ.get('WEATHER_XRESTORMER_TILE_OVERLAP', str(tile_cfg.get('tile_overlap', 64))))
                tile_cfg['tile_size'] = max(64, tile_size)
                tile_cfg['tile_overlap'] = max(0, tile_overlap)
            cfg['num_gpu'] = int(os.environ.get('WEATHER_XRESTORMER_NUM_GPU', '1'))
...
class XRestormer(BasicSRModel):
    """[A Comparative Study of Image Restoration Networks for General Backbone Network Design (ECCV 2024)](https://arxiv.org/abs/2310.11881) for SR, denoising, dehazing, deraining.

    Args:
        subtask (str): Subtask that can be handled by X-Restormer, one of `super_resolution`, `denoising`, `dehazing`, and `deraining`.
    """
    ...
    def _update_pretrained_ckpt(self, cfg: dict):
        ...
        cfg['path']['pretrain_network_g'] = str(project_root / f"{CKPT_DIR_NAME}/{self.work_dir_name}/{ckpt_name}")
...
class Restormer(Tool):
    """[Restormer: Efficient Transformer for High-Resolution Image Restoration (CVPR 2022)](https://openaccess.thecvf.com/content/CVPR2022/papers/Zamir_Restormer_Efficient_Transformer_for_High-Resolution_Image_Restoration_CVPR_2022_paper.pdf) for denoising, defocus deblurring, and deraining.

    Args:
        subtask (str): Subtask that can be handled by Restormer, one of `denoising`, `defocus_deblurring`, and `deraining`.
    """
    ...
    def _get_cmd_opts(self) -> list[str]:
        ...
        tile = os.environ.get('WEATHER_RESTORMER_TILE', '512')
        tile_overlap = os.environ.get('WEATHER_RESTORMER_TILE_OVERLAP', '32')
        return [
            "--task", self.opt_task,
            ...
            "--ckpt", str(project_root / f"{CKPT_DIR_NAME}/Restormer/{self.opt_task.lower()}.pth"),
        ]
...
class MPRNet(Tool):
    """[Multi-Stage Progressive Image Restoration (CVPR 2021)](https://openaccess.thecvf.com/content/CVPR2021/papers/Zamir_Multi-Stage_Progressive_Image_Restoration_CVPR_2021_paper.pdf) for denoising and deraining.

    Args:
        subtask (str): Subtask that can be handled by MPRNet, one of `denoising` and `deraining`.
    """
    ...
    def _get_cmd_opts(self) -> list[str]:
        """Requires parameter `input_dir: Path`, `output_dir: Path`, and `opt_task: str`."""
        tile = os.environ.get('WEATHER_MPRNET_TILE', '512')
        tile_overlap = os.environ.get('WEATHER_MPRNET_TILE_OVERLAP', '32')
        return [
            "--task", self.opt_task,
            "--input_dir", self.input_dir,
            "--result_dir", self.output_dir,
            "--model_arch_file", str(project_root / f"{UTILS_DIR_NAME}/denoising/tools/MPRNet/{self.opt_task}/MPRNet.py"),
            "--ckpt", str(project_root / f"{CKPT_DIR_NAME}/MPRNet/model_{self.opt_task.lower()}.pth"),
            "--tile", tile,
            "--tile_overlap", tile_overlap
        ]
...
class MAXIM(Tool):
    """[MAXIM: Multi-Axis MLP for Image Processing (CVPR 2022)](https://openaccess.thecvf.com/content/CVPR2022/papers/Tu_MAXIM_Multi-Axis_MLP_for_Image_Processing_CVPR_2022_paper.pdf) for denoising, deraining, raindrop removal, dehazing, low light enhancement, and image retouching.

    Args:
        subtask (str): Subtask that can be handled by MAXIM, one of `denoising`, `deraining`, and `dehazing`.
    """
    ...
    def _get_cmd_opts(self) -> list[str]:
        ...
        opts = [
            "--task", self.opt_task,
            "--ckpt_path", str(project_root / f"{CKPT_DIR_NAME}/MAXIM/{self.opt_ckpt_name}"),
            "--input_dir", self.input_dir,
            "--output_dir", self.output_dir,
            "--has_target=False"
        ]

        enable_tiling = os.environ.get('WEATHER_MAXIM_ENABLE_MULTI_GPU_TILING', '1').strip().lower() not in {'0', 'false', 'no'}
        if enable_tiling:
            opts.append('--enable_multi_gpu_tiling')
            gpu_ids = os.environ.get('WEATHER_MAXIM_GPU_IDS', '').strip()
            if gpu_ids:
                opts += ['--gpu_ids', gpu_ids]
            tile_size = os.environ.get('WEATHER_MAXIM_TILE_SIZE', '').strip()
            if tile_size:
                opts += ['--tile_size', tile_size]
            tile_overlap = os.environ.get('WEATHER_MAXIM_TILE_OVERLAP', '').strip()
            if tile_overlap:
                opts += ['--tile_overlap', tile_overlap]
            trigger = os.environ.get('WEATHER_MAXIM_TILE_TRIGGER_LONG_SIDE', '').strip()
            if trigger:
                opts += ['--tile_trigger_long_side', trigger]
...
class AutoDIR(Tool):
    ...
    def _get_cmd_opts(self) -> list[str]:
        ...
        return [
            ...
            "--ckpt", str(project_root / f"{CKPT_DIR_NAME}/AutoDIR/autodir.ckpt"),
            "--cfg-text", 1,
            "--config", str(project_root / f"{UTILS_DIR_NAME}/defocus_deblurring/tools/AutoDIR/configs/generate.yaml"),
            "--need-resize"
        ]
...
class DiffPlugin(Tool):
    ...
    def _get_cmd_opts(self) -> list[str]:
        ...
        if self.subtask == "defocus_deblurring":
            ckpt_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/deblur")
        elif self.subtask == "deraining":
            ckpt_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/derain")
        elif self.subtask == "desnowing":
            ckpt_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/desnow")
        elif self.subtask == "dehazing":
            ckpt_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/dehaze")
        elif self.subtask == "brightening":
            ckpt_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/lowlight")

        local_cache_dir = str(project_root / f"{CKPT_DIR_NAME}/Diff-Plugin")
        local_sd_v14_dir = project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/stable-diffusion-v1-4"
        local_sd_v12_dir = project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/stable-diffusion-v1-2"
        local_clip_dir = project_root / f"{CKPT_DIR_NAME}/Diff-Plugin/clip-vit-large-patch14"

        local_sd_dir = local_sd_v14_dir if local_sd_v14_dir.exists() else local_sd_v12_dir
        local_sd_ok = local_sd_dir.exists()
        local_clip_ok = local_clip_dir.exists()

        # Prefer local cache if complete, otherwise allow HF fallback/global cache.
        pretrained_src = str(local_sd_dir) if local_sd_ok else "CompVis/stable-diffusion-v1-4"
        clip_src = str(local_clip_dir) if local_clip_ok else "openai/clip-vit-large-patch14"
        local_only = local_sd_ok and local_clip_ok
        ...
        if local_only:
            opts.append("--local_files_only")

        enable_tiling = os.environ.get('WEATHER_DIFFPLUGIN_ENABLE_MULTI_GPU_TILING', '1').strip().lower() not in {'0', 'false', 'no'}
        if enable_tiling:
            opts.append('--enable_multi_gpu_tiling')
            # Let diffplugin auto-select from visible GPUs (CUDA_VISIBLE_DEVICES).
            # Do NOT pass --gpu_ids physical IDs that don't match the remapped devices.
            tile_size = os.environ.get('WEATHER_DIFFPLUGIN_TILE_SIZE', '').strip()
            if tile_size:
                opts += ['--tile_size', tile_size]
            tile_overlap = os.environ.get('WEATHER_DIFFPLUGIN_TILE_OVERLAP', '').strip()
            if tile_overlap:
                opts += ['--tile_overlap', tile_overlap]
            trigger = os.environ.get('WEATHER_DIFFPLUGIN_TILE_TRIGGER_LONG_SIDE', '').strip()
            if trigger:
                opts += ['--tile_trigger_long_side', trigger]
        return opts
