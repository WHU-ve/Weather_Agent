...
# Pillow>=10 removed _RESAMPLE_LANCZOS; keep compatibility for old/new versions.
_RESAMPLE_LANCZOS = getattr(getattr(Image, 'Resampling', Image), 'LANCZOS', getattr(Image, 'ANTIALIAS', Image.BICUBIC))
...
class ValData(data.Dataset):
    ...
    def get_images(self, index):
        ...
        input_img = input_img.resize((wd_new,ht_new), _RESAMPLE_LANCZOS)
        gt_img = gt_img.resize((wd_new, ht_new), _RESAMPLE_LANCZOS)
...
class ValData_unpaired_demo(data.Dataset):
    def __init__(self, val_data_dir,val_filename):
    # def __init__(self, val_data_dir):
        super().__init__()
        val_list = glob(os.path.join(val_data_dir, "*"))
        
        input_names=[]
        for val_image_path in val_list:
            contents = os.path.basename(val_image_path)
            input_names.append(contents)
        ...
        # self.val_data_dir_ = val_data_dir
