...
Weather-specific quality indicators: Rain residual score (lower streak energy → better rain removal)、Fog density score / FADE (lower means less fog)、Local contrast (higher means better visibility)、Bright artifact ratio (lower means fewer snow artifacts)、Edge continuity (higher means better structural recovery). Typically, high Rain residual score suggests rain degradation, high Bright artifact ratio suggests snow degradation. When cues conflict, output all plausible degradations rather than forcing a single label.
...
def _estimate_weather_indicators(image: Image.Image) -> Dict[str, float]:
    ...
    fade = fog_density
    bright_artifact_ratio = float(np.clip(bright_ratio, 0.0, 1.0))
    ...
    return {
        ...
        'bright_artifact_ratio': bright_artifact_ratio,
...
def _build_iqa_result(image: Image.Image, iqa_result: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"Bright artifact ratio={weather_indicators['bright_artifact_ratio']:.4f}, "
        f"Edge continuity={weather_indicators['edge_continuity']:.4f}"
    )
...
def _fallback_vector(image: Image.Image) -> np.ndarray:
    ...
    snow = float(np.clip(indicators['bright_artifact_ratio'] * 0.8 + (1.0 - indicators['edge_continuity']) * 0.2, 0.0, 1.0))
