...
class QualityEvaluator:
    ...
    SHARED_BOUNDS_15 = {
        ...
        'detail_score': (0.08, 0.70),
        'texture_retention': (0.05, 0.50),
    }
    ...
    def __init__(self, weights=None, normalize=True, norm_method='minmax'):
        ...
        # Fusion defaults. Data-driven: pure task_score (alpha=1.0) is optimal for all tasks.
        # General IQA is kept available but disabled by default.
        self.alpha_task = float(np.clip(float(os.getenv('QE_ALPHA_TASK', '1.0')), 0.0, 1.0))
        self.alpha_by_task = {
            'derain': float(np.clip(float(os.getenv('QE_ALPHA_DERAIN', '0.90')), 0.0, 1.0)),
            'dehaze': float(np.clip(float(os.getenv('QE_ALPHA_DEHAZE', '0.80')), 0.0, 1.0)),
            'desnow': float(np.clip(float(os.getenv('QE_ALPHA_DESNOW', '0.40')), 0.0, 1.0)),
        }
        ...
        # Per-task formula weights.
        # Five sub-metrics: main_residual, delta_improvement, local_contrast, detail_score, texture_retention
        self.w_rain_main = float(os.getenv('QE_RAIN_W_MAIN', '0.30'))
        self.w_rain_delta = float(os.getenv('QE_RAIN_W_DELTA', '0.40'))
        self.w_rain_lc = float(os.getenv('QE_RAIN_W_LOCAL_CONTRAST', '0.08'))
        self.w_rain_detail = float(os.getenv('QE_RAIN_W_DETAIL', '0.12'))
        self.w_rain_texture = float(os.getenv('QE_RAIN_W_TEXTURE', '0.10'))

        self.w_haze_main = float(os.getenv('QE_HAZE_W_MAIN', '0.20'))
        self.w_haze_delta = float(os.getenv('QE_HAZE_W_DELTA', '0.40'))
        self.w_haze_lc = float(os.getenv('QE_HAZE_W_LOCAL_CONTRAST', '0.05'))
        self.w_haze_detail = float(os.getenv('QE_HAZE_W_DETAIL', '0.20'))
        self.w_haze_texture = float(os.getenv('QE_HAZE_W_TEXTURE', '0.15'))

        self.w_snow_main = float(os.getenv('QE_SNOW_W_MAIN', '0.30'))
        self.w_snow_delta = float(os.getenv('QE_SNOW_W_DELTA', '0.35'))
        self.w_snow_lc = float(os.getenv('QE_SNOW_W_LOCAL_CONTRAST', '0.08'))
        self.w_snow_detail = float(os.getenv('QE_SNOW_W_DETAIL', '0.15'))
        self.w_snow_texture = float(os.getenv('QE_SNOW_W_TEXTURE', '0.12'))
    ...
    def _estimate_weather_indicators(image: Image.Image) -> Dict[str, float]:
        ...
        # detail_score: P90 gradient magnitude — higher means stronger edges (clear image).
        detail_score = float(np.clip(np.percentile(grad_mag, 90) * 6.0, 0.0, 1.0))

        # snow_artifact: isolated bright spots (snow-like) vs large bright areas (clouds/walls).
        bright_mask = (gray > 0.92).astype(np.float32)
        # Count bright neighbors in 3x3 window via manual shifts.
        h, w = gray.shape
        ...
        # texture_retention: mean local std in 5x5 patches — higher means more texture preserved.
        patch_size = 5
        pad = patch_size // 2
    ...
    def _extract_features(self, image_path: str) -> Dict[str, float]:
        ...
        features = {
            ...
            'snow_artifact': weather['snow_artifact'],
            'local_contrast': weather['local_contrast'],
            'detail_score': weather['detail_score'],
            'texture_retention': weather['texture_retention'],
        }
    ...
    def _task_score(self, task_name: Optional[str], row: Dict[str, float],
                    input_features: Optional[Dict[str, float]] = None) -> Tuple[float, Dict[str, float]]:
        key = (task_name or '').strip().lower()
        ...
        if key == 'dehaze':
            ...
            if input_features is not None:
                input_val = input_features.get(main_key)
                output_val = row.get(main_key)
                if input_val is not None and output_val is not None and input_val > 1e-6:
                    reduction = (input_val - output_val) / input_val
                    delta_util = float(np.clip(reduction, 0.0, 1.0))
                    delta_rel = 0.5

            lc_util, lc_rel = self._metric_unit_and_reliability('local_contrast', row)
            detail_util, detail_rel = self._metric_unit_and_reliability('detail_score', row)
            texture_util, texture_rel = self._metric_unit_and_reliability('texture_retention', row)

            if input_features is not None:
                for metric_key, attr_name in [('detail_score', 'detail'), ('texture_retention', 'texture')]:
                    input_val = input_features.get(metric_key)
                    output_val = row.get(metric_key)
                    if input_val is not None and output_val is not None and input_val > 1e-6:
                        ratio = output_val / input_val
                        if attr_name == 'detail':
                            detail_util = float(np.clip(ratio, 0.0, 1.0))
                            detail_rel = 0.6
                        else:
                            texture_util = float(np.clip(ratio, 0.0, 1.0))
                            texture_rel = 0.6

            base = [self.w_haze_main, self.w_haze_delta, self.w_haze_lc, self.w_haze_detail, self.w_haze_texture]
            rels = [main_rel, delta_rel, lc_rel, detail_rel, texture_rel]
            w_main, w_delta, w_lc, w_detail, w_texture = self._modulate_weights(base, rels)

            score = float(w_main * main_good + w_delta * delta_util + w_lc * lc_util
                          + w_detail * detail_util + w_texture * texture_util)
            details = {
                'main_util': main_good,
                'delta_util': delta_util,
                'lc_util': lc_util,
                'detail_util': detail_util,
                'texture_util': texture_util,
                'main_weight': w_main,
                'delta_weight': w_delta,
                'lc_weight': w_lc,
                'detail_weight': w_detail,
        ...
        # Desnow: adaptive scoring based on input image quality.
        # Clear input → protect detail (conservative). Blurry input → remove snow (aggressive).
        if key == 'desnow':
            main_key = self.TASK_MAIN_KEY[key]
            main_util, main_rel = self._metric_unit_and_reliability(main_key, row, task_name=key)
            main_good = 1.0 - main_util
            ...
            detail_util, detail_rel = self._metric_unit_and_reliability('detail_score', row)
            texture_util, texture_rel = self._metric_unit_and_reliability('texture_retention', row)
            detail_rel_v, texture_rel_v = detail_rel, texture_rel
            if input_features is not None:
                for mk, an in [('detail_score','detail'),('texture_retention','texture')]:
                    iv = input_features.get(mk, 0); ov = row.get(mk, 0)
                    if iv > 1e-6:
                        ratio = float(np.clip(ov / iv, 0.0, 1.0))
                        if an == 'detail': detail_util, detail_rel_v = ratio, 0.6
                        else: texture_util, texture_rel_v = ratio, 0.6
            delta_util = 0.0; delta_rel = 0.0
            if input_features is not None:
                iv = input_features.get(main_key, 0); ov = row.get(main_key, 0)
                if iv > 1e-6:
                    delta_util = float(np.clip((iv - ov) / iv, 0.0, 1.0)); delta_rel = 0.5

            # Fixed weights: reward snow removal but also protect detail/texture.
            base = [0.17, 0.30, 0.02, 0.35, 0.15]  # main, delta, lc, detail, texture
            rels = [main_rel, delta_rel, lc_rel, detail_rel_v, texture_rel_v]
            w_main, w_delta, w_lc, w_detail, w_texture = self._modulate_weights(base, rels)

            score = float(w_main * main_good + w_delta * delta_util + w_lc * lc_util
                          + w_detail * detail_util + w_texture * texture_util)
            details = {
                'main_util': main_good, 'delta_util': delta_util,
                'lc_util': lc_util, 'detail_util': detail_util, 'texture_util': texture_util,
                'main_weight': w_main, 'delta_weight': w_delta,
                'lc_weight': w_lc, 'detail_weight': w_detail, 'texture_weight': w_texture,
                'main_rel': main_rel, 'delta_rel': delta_rel,
                'lc_rel': lc_rel, 'detail_rel': detail_rel_v, 'texture_rel': texture_rel_v,
                'main_key': main_key,
            }
            return float(score), details
        ...
        # Delta: relative reduction of the main degradation metric from input to output.
        delta_util = 0.0
        delta_rel = 0.0
        if input_features is not None:
            input_val = input_features.get(main_key)
        ...
        # Detail & texture as RELATIVE preservation (output/input), capped at 1.0.
        # This prevents under-processing experts from winning on absolute sharpness.
        detail_util, detail_rel = self._metric_unit_and_reliability('detail_score', row)
        texture_util, texture_rel = self._metric_unit_and_reliability('texture_retention', row)
        if input_features is not None:
            for metric_key, attr_name in [('detail_score', 'detail'), ('texture_retention', 'texture')]:
        ...
        if key == 'derain':
            base = [self.w_rain_main, self.w_rain_delta, self.w_rain_lc, self.w_rain_detail, self.w_rain_texture]
        elif key == 'dehaze':
            base = [self.w_haze_main, self.w_haze_delta, self.w_haze_lc, self.w_haze_detail, self.w_haze_texture]
        else:
            base = [self.w_snow_main, self.w_snow_delta, self.w_snow_lc, self.w_snow_detail, self.w_snow_texture]
    ...
    def evaluate(self, image_path, task_name: Optional[str] = None,
        ...
        task_score, _ = self._task_score(task_name, row, input_features=input_features)
    ...
    def select_best(self, candidate_paths, task_name: Optional[str] = None,
        ...
        for idx, row in enumerate(rows):
            task_score, task_details = self._task_score(task_name, row, input_features=input_features)
            ...
            details.append(
                {
                    ...
                    'task_delta_util': task_details.get('delta_util', 0.0),
                    'task_main_key': task_details.get('main_key', ''),
                    'task_main_weight': task_details['main_weight'],
                    'task_delta_weight': task_details.get('delta_weight', 0.0),
                    'task_lc_weight': task_details['lc_weight'],
                    'task_detail_weight': task_details.get('detail_weight', 0.0),
                    'task_texture_weight': task_details.get('texture_weight', 0.0),
                    'task_main_rel': task_details['main_rel'],
                    'task_lc_rel': task_details['lc_rel'],
                    'task_detail_rel': task_details.get('detail_rel', 0.0),
                    ...
                    'snow': float(row['snow_artifact']),
                    'lc': float(row['local_contrast']),
                    'detail': float(row['detail_score']),
                    'texture': float(row['texture_retention']),
                }
            )
        ...
        if self.verbose_selection:
            ...
            for item in sorted(details, key=lambda x: x['final'], reverse=True):
                print(
                    ...
                    f"main={item['task_main_key']} util={item['task_main_util']:.4f} delta={item['task_delta_util']:.4f}, "
                    f"w(main/delta/lc/detail/texture)=({item['task_main_weight']:.3f}/{item['task_delta_weight']:.3f}/"
                    f"{item['task_lc_weight']:.3f}/{item['task_detail_weight']:.3f}/{item['task_texture_weight']:.3f}), "
                    ...
                    f"LC={item['lc']:.4f}, detail={item['detail']:.4f}, texture={item['texture']:.4f}"
                )
