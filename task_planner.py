...
class TaskPlanner:
    ...
    def _plan_hybrid(self, degradation_vector, image_path: Optional[str] = None):
        ...
        # perception_direct 模式：直接使用感知模块输出的退化顺序（不重排序）
        if self.planner_mode == 'perception_direct':
            if isinstance(degradation_vector, dict):
                raw = degradation_vector.get('degradations', []) or []
                ordered = []
                for item in raw:
                    d = str(item.get('type', item) if isinstance(item, dict) else item).strip().lower()
                    d = {'fog': 'haze', 'smog': 'haze', 'mist': 'haze', 'rainy': 'rain', 'snowy': 'snow'}.get(d, d)
                    t = self.label_to_step.get(d)
                    if t and t not in ordered:
                        ordered.append(t)
                if ordered:
                    self.last_plan_metadata['planner_source'] = 'perception_direct_raw_order'
                    return ordered
            return direct_plan
    ...
    def build_local_replan_prompt(
        ...
        return (
            ...
            'Generate a complete plan for all remaining unfinished tasks, reordered as needed. Output JSON only.\n\n'
            f'Input image description: {image_description}\n'
            f'Input detected degradations: {degrade_text}\n'
            f'Completed tasks that must NOT appear again: {completed_text}\n'
            f'Candidate unfinished tasks for the new plan: {remaining_text}\n'
            f'Tasks that failed when placed first in the sequence (cannot be first again): {failed_first_text}\n'
            f'Must-check unresolved degradations for this replan round: {must_check_text}\n'
            f'Current failed task: {failed_task} (still degradation: {failed_label})\n'
            f'Failure experiences: {tried_text}\n\n'
            'Rules:\n'
            '1) Return JSON with key "plan" only. The plan should contain the complete sequence of all remaining tasks.\n'
            '2) Use only candidate unfinished tasks; do not include completed tasks.\n'
            '3) The first task must NOT be in the "failed when placed first" list.\n'
            '4) A failed-first task may appear later (not as the first task), but only when the candidate unfinished tasks include at least one task that is not in the failed-first list.\n'
            '5) The first task should address the input detected degradations and the must-check unresolved degradations.\n'
            '6) If the first task is an alternative task, keep the failed task later unless it is completed or no longer useful.\n'
            '7) Keep plan concise and deterministic.\n'
        )
