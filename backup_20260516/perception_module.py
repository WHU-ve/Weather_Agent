...
Weather-specific quality indicators (computed directly from the image): Rain residual score (horizontal vs vertical gradient asymmetry; higher → rain streaks present)、Fog density score / FADE (1 − local contrast + bright region ratio; higher → more haze)、Local contrast (gray-level stddev; lower → less visibility, haze likely)、Snow artifact score (isolated bright pixel ratio; higher → snow specks present)、Detail score (P90 gradient magnitude; lower → blur)、Texture retention (local 5×5 std mean; lower → over-smoothed). Degradation hints: high rain_residual → rain; high fog_density + low local_contrast → haze; high snow_artifact → snow. When cues conflict, output all plausible degradations.
