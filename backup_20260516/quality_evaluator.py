...
class QualityEvaluator:
    ...
    def __init__(self, weights=None, normalize=True, norm_method='minmax'):
        ...
        self.alpha_by_task = {
            'derain': float(np.clip(float(os.getenv('QE_ALPHA_DERAIN', '1.0')), 0.0, 1.0)),
            'dehaze': float(np.clip(float(os.getenv('QE_ALPHA_DEHAZE', '0.80')), 0.0, 1.0)),
            'desnow': float(np.clip(float(os.getenv('QE_ALPHA_DESNOW', '1.0')), 0.0, 1.0)),
        }
        ...
        self.w_haze_main = float(os.getenv('QE_HAZE_W_MAIN', '0.30'))
        self.w_haze_delta = float(os.getenv('QE_HAZE_W_DELTA', '0.30'))
        self.w_haze_lc = float(os.getenv('QE_HAZE_W_LOCAL_CONTRAST', '0.20'))
        self.w_haze_detail = float(os.getenv('QE_HAZE_W_DETAIL', '0.10'))
        self.w_haze_texture = float(os.getenv('QE_HAZE_W_TEXTURE', '0.10'))
    ...
    def _task_score(self, task_name: Optional[str], row: Dict[str, float],
        ...
        # Dehaze uses the original formula: fog_density + lc + edge proxy.
        if key == 'dehaze':
            ...
            base = [0.55, 0.30, 0.15]
            rels = [main_rel, lc_rel, detail_rel]
            w_main, w_lc, w_detail = self._modulate_weights(base, rels)

            score = float(w_main * main_good + w_lc * lc_util + w_detail * detail_util)
            details = {
                'main_util': main_good,
                'delta_util': 0.0,
                'lc_util': lc_util,
        ...
        # Desnow uses a simple degradation-reduction formula.
        # Data-driven: main_good + delta alone is optimal; penalties hurt.
        if key == 'desnow':
            main_key = self.TASK_MAIN_KEY[key]
            main_util, main_rel = self._metric_unit_and_reliability(main_key, row, task_name=key)
            main_good = 1.0 - main_util
            ...
            if input_features is not None:
                ...
                if input_val is not None and output_val is not None and input_val > 1e-6:
                    delta_util = float(np.clip((input_val - output_val) / input_val, 0.0, 1.0))
                    delta_rel = 0.5
            score = 0.5 * main_good + 0.5 * delta_util
            details = {
                'main_util': main_good,
