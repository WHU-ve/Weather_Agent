...
OUT_ROOT = PROJECT_ROOT / 'output' / 'tmp_profile_12samples_dynamic_noreplan'
...
def run_profile():
    ...
    for idx, (task, d) in enumerate(task_dirs.items()):
        picked = _sample_inputs(d, k=4, seed=base_seed + idx)
        for p in picked:
            samples.append((task, p))

    _log('=== CONFIG (FULLCHAIN MULTI-EXPERT, NO DYNAMIC REPLAN) ===')
    _log(f'Samples(12): {[f"{t}:{p.name}" for t, p in samples]}')
