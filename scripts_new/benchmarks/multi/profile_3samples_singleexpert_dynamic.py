...
OUT_ROOT = PROJECT_ROOT / 'output' / 'tmp_profile_6samples_singleexpert_dynamic'
...
def run_profile():
    ...
    for idx, (task, d) in enumerate(task_dirs.items()):
        picked = _sample_inputs(d, k=2, seed=base_seed + idx)
        for p in picked:
    ...
    _log(f'Samples(6): {[f"{t}:{p.name}" for t, p in samples]}')
    ...
    for idx, (task, inp) in enumerate(samples, 1):
        ...
        _log(f'[{idx}/6] START {task}:{inp.name}')
        t0 = time.time()
        out_buf = io.StringIO()
        err_buf = io.StringIO()
        ...
        if not inp.exists():
            ...
            rows.append(row)
            _log(f'[{idx}/6] FAIL {task}:{inp.name} input_not_found')
            continue
        ...
        if ok:
            _log(f'[{idx}/6] DONE {task}:{inp.name} t={dt:.1f}s peak={peak_mb:.0f}MB experts={experts} replan={replan}')
        else:
            _log(f'[{idx}/6] FAIL {task}:{inp.name} t={dt:.1f}s err={err}')
            if stdout_txt.strip():
                _log(f'[{idx}/6] stdout_tail:\n' + '\n'.join(stdout_txt.splitlines()[-10:]))
            if stderr_txt.strip():
                _log(f'[{idx}/6] stderr_tail:\n' + '\n'.join(stderr_txt.splitlines()[-10:]))
    ...
    summary = {
        ...
        'sampling': {'per_task': 2, 'seed_base': base_seed},
    }

    out = {'summary': summary, 'rows': rows}
    out_fp = OUT_ROOT / 'profile_6samples_summary.json'
