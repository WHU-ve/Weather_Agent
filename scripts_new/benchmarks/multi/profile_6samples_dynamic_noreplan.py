...
OUT_ROOT = PROJECT_ROOT / 'output' / 'tmp_profile_6samples_dynamic_noreplan'
...
def run_profile():
    ...
    _log('=== CONFIG (FULLCHAIN MULTI-EXPERT, NO DYNAMIC REPLAN) ===')
    _log(f'Samples(6): {[f"{t}:{p.name}" for t, p in samples]}')

    # 保留感知 + 规划 + 多专家；仅关闭动态重规划
    ...
    summary = {
        ...
        'experts_per_img_mean': statistics.mean(ex) if ex else 0.0,
